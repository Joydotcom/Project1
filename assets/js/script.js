...
console.log($("#optionState").val())
     ...
     fetch("https://developer.nps.gov/api/v1/activities/parks?id=&q=hiking&limit=5&start=0&api_key=7Ir4CEYWv3mXsoB7yi5AYKGSBLXqPI5lOBOUvet2")
       .then(response => {
           return response.json();
       })  .then(function(data){
          ...
          for (var i = 0; i < data.data[0].parks.length; i++) {
             ...
             if ($("#optionState").val() === data.data[0].parks[i].states) {
                  console.log("This is a match for " + data.data[0].parks[i].states + "state")
                  console.log("This is the park that matches it " + data.data[0].parks[i].fullName)
          }
   ...
   var activities = ["camping", "hiking", "biking", "fishing", "guided tours", "skiing", "swimming", "wildlife watching"]
